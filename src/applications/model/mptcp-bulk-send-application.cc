...
MpTcpBulkSendApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MpTcpBulkSendApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
    .AddAttribute ("MaxSubflows", "Maximum number of subflows made by woody",
                   UintegerValue (2),
                   MakeUintegerAccessor (&MpTcpBulkSendApplication::m_maxSubflows),
                   MakeUintegerChecker<uint32_t> ())

      .AddAttribute("CongestionControl",
                    "Congestion control algorithm",
          EnumValue(Linked_Increases),
          MakeEnumAccessor(&MpTcpBulkSendApplication::SetCongestionCtrlAlgo),
...
void
MpTcpBulkSendApplication::SetCongestionCtrlAlgo(CongestionCtrl_t ccalgo)
{
  AlgoCC = ccalgo;
}
  ...
  if (!m_socket)
    {
      ...
      m_socket->SetMaxSubFlowNumber(m_maxSubflows);
      m_socket->SetCongestionCtrlAlgo(AlgoCC);
      m_socket->Bind();
