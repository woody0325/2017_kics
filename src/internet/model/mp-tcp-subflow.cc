  ...
  if (m_epochStart <= 0)
        {
          ...
          if (cwnd < lastMax)
                {
                  m_k = pow((lastMax - cwnd) / 0.4, (1.0 / 3.0));
                  m_originPoint = lastMax;
                }
          else
                {
                  m_k = 0.0;
...
//        m_wTcp = cwnd;
  ...
  double t = (Simulator::Now () + RTT_min - m_epochStart).GetSeconds ();
  double target = m_originPoint + 0.4 * pow(t - m_k, 3.0);
...
void
MpTcpSubFlow::CubicReset(void)
{
  lastMax = 0;
  m_epochStart = Time("0.0s");
  m_originPoint = 0;
  ackCnt = 0;
  m_lastTime = Time("0.0s");
}
