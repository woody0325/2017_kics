  ...
  static TypeId tid = TypeId("ns3::MpTcpSocketBase")
      ...
      .AddAttribute("CongestionControl",
          ...
          MakeEnumChecker(Uncoupled_TCPs,   "Uncoupled_TCPs",
                          ...
                          UNCOUPLED, "UNCOUPLED",
                          Uncoupled_Cubics, "Uncoupled_Cubics"))
...
MpTcpSocketBase::CompleteFork(Ptr<Packet> p, const TcpHeader& mptcpHeader, const Address& fromAddress, const Address& toAddress)
{
  ...
  NS_LOG_INFO("CompleteFork -> RxSeqNb: " << sFlow->RxSeqNumber << " highestAck: " << sFlow->highestAck);
...
MpTcpSocketBase::ReceivedAck(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
{
  ...
  // Received ACK. Compare the ACK number against highest unacked seqno.
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer (DSNMapping List)
      NS_LOG_INFO ("New ack of " << mptcpHeader.GetAckNumber ());
...
MpTcpSocketBase::DoRetransmit(uint8_t sFlowIdx, DSNMapping* ptrDSN)
{
  ...
  NS_LOG_INFO (Simulator::Now().GetSeconds() <<" RetransmitSegment -> "<< " localToken "<< localToken<<" Subflow "<<(int) sFlowIdx<<" DataSeq "<< ptrDSN->dataSeqNumber <<" SubflowSeq " << ptrDSN->subflowSeqNumber <<" dataLength " << ptrDSN->dataLevelLength << " packet size " << pkt->GetSize() << " 3DupACK");
  uint8_t hlen = 5;
  uint8_t olen = 20; //uint8_t olen = 15;
  uint8_t plen = 0;
  plen = (4 - (olen % 4)) % 4;
        ...
        { // SendingBuffer is empty but subflowBuffer (mapDSN) is not. Also subflow is recovering from timeOut.
          ...
          while (window != 0 && window >= sF->MSS && sF->maxSeqNb > sF->TxSeqNumber - 1 && sF->mapDSN.size() > 0)
            { // In case case more than one packet can be sent, if subflow's window allow
              whileCounter++;
              NS_LOG_INFO("["<< m_node->GetId() <<"] MainBuffer is empty - subflowBuffer(" << sF->mapDSN.size()<< ") sFlow("<< (int)sFlowIdx << ") AvailableWindow: " << window << " CWND: " << sF->cwnd << " subflow is in timoutRecovery{" << (sF->mapDSN.size() > 0) << "} LoopIter: " << whileCounter);
...
MpTcpSocketBase::ReduceCWND(uint8_t sFlowIdx, DSNMapping* ptrDSN)
{
  ...
  case Uncoupled_Cubics:
    sFlow->cwnd  = sFlow->ssthresh + 3 * mss; //((double) sFlow->ssthresh/((double) sFlow->MSS)) + 3 ;
    break;
...
MpTcpSocketBase::NewACK(uint8_t sFlowIdx, const TcpHeader& mptcpHeader, TcpOptions* opt)
{
  ...
  NS_LOG_INFO("NewACK-> sFlow->highestAck: " << sFlow->highestAck);
  ...
  SendPendingData(sFlow->routeId); // in newack()
                ...
                {
                                case Uncoupled_Cubics :
                                          sFlow->m_epochStart = Time();
                                          if (Simulator::Now () > sFlow->lastMax+ 0.1 * sFlow->m_k)
                                                {
                                                  if (sFlow->cwnd <  sFlow->lastMax )
                                                        {
                                                           sFlow->lastMax= sFlow->cwnd * (2.0 - 0.2) / 2.0;
                                                        }
                                                  else
                                                        {
                                                           sFlow->lastMax = sFlow->cwnd;
                                                        }
                                                }
                                          sFlow->m_lastTime= Simulator::Now ();

                                          sFlow->cwnd = sFlow->cwnd * (1.0 - 0.2);
//                                        if (sFlow->cwnd <= 1)
//                                              sFlow->cwnd = 2;
//                                    m_ssThresh = sFlow->cwnd;
                                      //m_cWnd = m_ssThresh;
                                      sFlow->m_inFastRec = false;
                          break;
			default:
  ...
  if (hasSyn)
    {
      if (sFlow->cnCount == 0)
        ...
        { // Exponential backoff of connection time out
          ...
          NS_LOG_INFO(Simulator::Now().GetSeconds() << " ["<< m_node->GetId() << "] ("<< (int)sFlow->routeId<< ") " << flowType << " SendEmptyPacket -> backoffCount: " << backoffCount << " RTO: " << RTO.GetSeconds() << " cnTimeout: " << sFlow->cnTimeout.GetSeconds() <<" cnCount: "<< sFlow->cnCount);
        }
    }
  if (((sFlow->state == SYN_SENT) || (sFlow->state == SYN_RCVD && mpEnabled == true)) && mpSendState == MP_NONE)
    {
      ...
      NS_LOG_INFO("["<< m_node->GetId() << "] ("<< (int)sFlow->routeId<< ") SendEmptyPacket -> LOCALTOKEN is mapped to connection endpoint -> " << localToken << " -> " << m_endPoint << " TokenMapsSize: "<< m_tcp->m_TokenMap.size());
    }
      ...
      if (hasSyn)
        {
          ...
          NS_LOG_INFO (this << " ["<< m_node->GetId() << "]("<<(int)sFlowIdx <<") SendEmptyPacket -> "<< TcpFlagPrinter(flags)<< " ReTxTimer set for SYN / SYN+ACK now " << Simulator::Now ().GetSeconds () << " Expire at " << (Simulator::Now () + RTO).GetSeconds () << " RTO: " << RTO.GetSeconds() << " FlowType: " << flowType << " Header: "<< header);
        }
      if (hasFin)
        {
          NS_LOG_INFO (this << " ["<< m_node->GetId() << "]("<<(int)sFlowIdx <<") SendEmptyPacket -> "<<TcpFlagPrinter(flags)<< " ReTxTimer set for FIN / FIN+ACK now " << Simulator::Now ().GetSeconds () << " Expire at " << (Simulator::Now () + RTO).GetSeconds () << " RTO: " << RTO.GetSeconds() << " FlowType: " << flowType << " Header: " << header);
        }
    }
...
MpTcpSocketBase::InitiateSubflows()
{
  ...
  NS_LOG_ERROR(this << " subflow size " << subflows.size()); // woody
...
// Create new subflow
  ...
  NS_LOG_ERROR(this << m_node->GetId() << " randomPort: " << randomPort); // woody
  Ptr<MpTcpSubFlow> sFlow = CreateObject<MpTcpSubFlow>();
  sFlow->routeId = (subflows.size() == 0 ? 0 : subflows[subflows.size() - 1]->routeId + 1);
...
{
  NS_LOG_FUNCTION_NOARGS();
  NS_LOG_ERROR(this << " DRAND_GAP case"); // woody
  //Ptr<UniformRandomVariable> uniRandom = CreateObject<UniformRandomVariable>();
  //Ptr<UniformRandomVariable> uniRandom = CreateObject<UniformRandomVariable>();
  vector<uint32_t> randomStorage;
  uint16_t randomPort = 0;
...
MpTcpSocketBase::DupAck(uint8_t sFlowIdx, DSNMapping* ptrDSN)
{
  ...
  if (sFlow->m_dupAckCount == 3 && !sFlow->m_inFastRec)
    { // FastRetrasmsion
      NS_LOG_INFO (Simulator::Now().GetSeconds() <<" DupAck -> Subflow ("<< (int)sFlowIdx <<") 3rd duplicated ACK for segment ("<<ptrDSN->subflowSeqNumber<<")");
...
// Increase cwnd for every additional DupACK (RFC2582, sec.3 bullet #3)
      ...
      sFlow->ssthreshtrack.push_back(make_pair(Simulator::Now().GetSeconds(), sFlow->ssthresh));
#endif
      NS_LOG_INFO ("DupAck-> FastRecovery. Increase cwnd by one MSS, from " << sFlow->cwnd.Get() <<" -> " << sFlow->cwnd << " AvailableWindow: " << AvailableWindow(sFlowIdx));
      FastRecoveries++;
    ...
    {
      NS_LOG_INFO("Limited transmit is not enabled... DupAcks: " << ptrDSN->dupAckCount);
    }
...
MpTcpSocketBase::AdvertiseAvailableAddresses()
{
  ...
  NS_LOG_ERROR("AdvertiseAvailableAddresses called " << this); // woody
  if (mpEnabled == true)
    {
...
/*
	...
	// additional advertising
        ...
        olen += 6;
        localAddrs.insert(localAddrs.end(), addrInfo);
	NS_LOG_ERROR("AdvertiseAvailableADDRESS additional one");
     }	
*/
...
MpTcpSocketBase::AvailableWindow(uint8_t sFlowIdx)
{
  ...
  if (freeCWND < sFlow->MSS && sendingBuffer.PendingData() >= sFlow->MSS)
    {
      NS_LOG_INFO("AvailableWindow: ("<< (int)sFlowIdx <<") -> " << freeCWND << " => 0" << " MSS: " << sFlow->MSS);
      return 0;
    }
  else
    {
      NS_LOG_INFO("AvailableWindow: ("<< (int)sFlowIdx <<") -> " << freeCWND );
      return freeCWND;
    }
}
...
// Recevier would create its new subflow when SYN with MP_JOIN being sent.
  ...
  NS_LOG_UNCOND("subflow size is " << subflows.size());
...
#endif
      NS_LOG_INFO ("Congestion Control (Slow Start) increment by one segmentSize");
    }
  else
    {
      switch (AlgoCC)
      ...
      case RTT_Compensator:
        ...
        totalCWNDtrack.push_back(make_pair(Simulator::Now().GetSeconds(), totalCwnd));
#endif
        NS_LOG_INFO ("Congestion Control (RTT_Compensator): alpha "<<alpha<<" ackedBytes (" << ackedBytes << ") totalCwnd ("<< totalCwnd / sFlow->MSS<<" packets) -> increment is "<<adder << " cwnd: " << sFlow->cwnd);
        break;
      case Linked_Increases:
        calculateAlpha();
        ...
        totalCWNDtrack.push_back(make_pair(Simulator::Now().GetSeconds(), totalCwnd));
#endif
        NS_LOG_INFO ("Subflow "<<(int)sFlowIdx<<" Congestion Control (Linked_Increases): alpha "<<alpha<<" increment is "<<adder<<" ssthresh "<< ssthresh << " cwnd "<<cwnd );
        break;
      case Uncoupled_TCPs:
        ...
        totalCWNDtrack.push_back(make_pair(Simulator::Now().GetSeconds(), totalCwnd));
#endif
        NS_LOG_INFO ("Subflow "<<(int)sFlowIdx<<" Congestion Control (Uncoupled_TCPs) increment is "<<adder<<" ssthresh "<< ssthresh << " cwnd "<<cwnd);
        break;
      case UNCOUPLED:
        sFlow->cwnd += tcp_inc;
        break;
      case Fully_Coupled:
        ...
        totalCWNDtrack.push_back(make_pair(Simulator::Now().GetSeconds(), totalCwnd));
#endif
        NS_LOG_INFO ("Subflow "<<(int)sFlowIdx<<" Congestion Control (Fully_Coupled) increment is "<<adder<<" ssthresh "<< ssthresh << " cwnd "<<cwnd);
        break;
      ...
      case COUPLED_FULLY:
//          sFlow->cwnd = cwnd + sFlow->calculate_cubic();
          total_cwnd = compute_total_window();
          ...
          break;
      case Uncoupled_Cubics:
          sFlow->cwnd = sFlow->cwnd + sFlow->calculate_cubic();
	  break;
      default:
        break;
        }
...
//  NS_LOG_ERROR (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " localToken "<< localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" Moving cwnd from " << cwnd << " to " << sFlow->cwnd <<" Throughput "<<(sFlow->cwnd * sFlow->MSS * 8)/rtt<< " GlobalThroughput "<<getGlobalThroughput()<< " Sum CWND ("<< totalCwnd <<")");
}
