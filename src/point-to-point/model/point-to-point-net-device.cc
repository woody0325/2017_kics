...
// woody
void
PointToPointNetDevice::stepBW(double alpha,Time duration)
{
  NS_LOG_ERROR(Simulator::Now().GetSeconds() << " stepBW with alpha " << alpha << " duration " << Seconds(duration));
  NS_LOG_ERROR("m_bps = " << m_bps);
  m_bps=DataRate(m_bps.GetBitRate()*alpha);
  NS_LOG_ERROR("m_bps = " << m_bps);
  Simulator::Schedule(duration,&PointToPointNetDevice::SetDataRate,this, DataRate(m_bps.GetBitRate()/alpha) );
}
...
PointToPointNetDevice::SetDataRate (DataRate bps)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_bps = bps;
  NS_LOG_ERROR(Simulator::Now().GetSeconds() << " m_bps " << m_bps);
}
