...
// woody
void
PointToPointHelper::StepDynamicBW (Ptr<NetDevice> nd ,double alpha, Time duration)//,int alpha
{
    NS_LOG_ERROR("StepDynamicBW, alpha " << alpha << " duration " << duration);
    Ptr<PointToPointNetDevice> device = nd->GetObject<PointToPointNetDevice> ();
    device->stepBW(alpha,duration);//alpha
}
